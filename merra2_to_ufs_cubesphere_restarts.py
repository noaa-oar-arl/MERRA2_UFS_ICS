...
def get_fv3_plevs(gfs_ctrl,return_akbk=False):
    ...
    """
    ak = gfs_ctrl.ak.squeeze().data

    bk = gfs_ctrl.bk.squeeze().data
...
#    grid = args.grid_spec
    ...
    #print(tracer_file.split('.')[-2].strip('tile'))
    tile = int(tracer_file.split('.')[-2].strip('tile'))
    core_tile = open_dataset(core_file.replace('.nc','.tile{}.nc'.format(tile)))
    ...
    for i in rename_dict:
        ...
        print('horizontal interpolation of ', field)
